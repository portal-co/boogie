...
func (s *State) cache(a string) (string, error) {
	s.CacheMtx.Lock()
	defer s.CacheMtx.Unlock()
	if s.CacheDir == "" {
		s.CacheDir = xdg.CacheHome + "/boogie-action-input"
	}
	err := os.MkdirAll(s.CacheDir, 0o777)
	if err != nil {
		return "", err
	}
	if _, err := os.Stat(s.CacheDir + "/" + a); err == nil {
		// path/to/whatever exists
		return s.CacheDir + "/" + a, nil
	} else if errors.Is(err, os.ErrNotExist) {
		// path/to/whatever does *not* exist
		err := remount.Clone(s.I, hos.NewFS(), a, (s.CacheDir + "/" + a)[1:])
		if err != nil {
			// fmt.Println(err)
			return "", err
			// continue
...
func (state *State) Run(inputs map[string]string, cmd []string, outs []string, append_ string) (string, error) {
	ix := inputs
	...
	for k, v := range ix {
		...
		err = os.Symlink(p, path.Join(t, k))
		if err != nil {
			return "", err
		}
	...
	return state.I.Push(hos.NewFS(), t[1:]+append_)
	// } else {
...
type Runner interface {
	Run(inputs map[string]string, cmd []string, outs []string, append string) (string, error)
	Ipfs() PFS
	IpfsPath() string
	IO() Stdio
...
// func (r Ramp[T]) Run(inputs map[string]string, cmd []string, outs []string, appens string) (string, error) {
// 	c := make(chan string)
// 	errs := map[T]error{}
// 	left := 0
